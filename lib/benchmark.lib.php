...
/**
 * The concept of this benchmarking suite is simple. You add checkpoints in your code
...
class Benchmark
{
	//! Prohibit the creation of benchmark objects
	 ...
	 * {	benchmark::checkpoint('my_slow_function start', $big_number);
	 * 
	 * 		for($i = 0; $i < $big_number;$i++)
	 * 		{	benchmark::checkpoint('my_slow_function iter', $i)
	 * 			$d = ($i * $i) + ($d/$i);
	 * 		}
	 * 
	 * 		benchmark::checkpoint('my_slow_function end', array($big_number, $d));
	 * 		return $d;
	 * }
	 * @endcode
	 */
	 ...
	 * for($i = 0;$i < 5;$i++)
	 * {
	 * 		benchmark::checkpoint('loop-start', $a);
	 * 		sleep(1.31);
	 * 		benchmark::checkpoint('loop-end');
	 * }
	 * @endcode
	 * 
	 * @b Output
	 ...
	 * @code
	 * function myadd($a, $b)
	 * {	benchmark::checkpoint('func_add', array($a, $b));
	 * 		$ret = $a + $b;
	 *		benchmark::checkpoint('func_end', $ret);
	 * }
	 * 
	 * echo myadd(1,2);
	 * var_dump(Benchmark::passes());
	 * @endcode
	 * 
	 * @b Output
	 * @code
	...
	{	$passes = self::passes($show_only, $exclude);
		...
		// Render html table
		echo '<table class="benchmark_dump_table" style="color: black; background: white;" border="1"><tr><th>Checkpoint <th>Variable <th>Program Time <th> Time Delta <th> Total Memory <th> Memory Delta' . "\n";
		foreach($passes as $pass)
